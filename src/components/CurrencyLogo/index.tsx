...
import tokenlist from 'elephantdexdefault-token-list'
...
export function getTokenlistURL(currency: any) {
  for (let i = 0; i < tokenlist.tokens.length; i++) {
    if (tokenlist.tokens[i].address.toString() === currency.address.toString()) {
      return tokenlist.tokens[i].logoURI
    }
  }
}
