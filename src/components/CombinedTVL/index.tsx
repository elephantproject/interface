  ...
  return (
    <>
      {TVLs?.stakingPoolTVL?.greaterThan('0') && (
        <CustomMouseoverTooltip
          element={
            <>
              {TVLs.stakingPoolTVL?.greaterThan('0') && (
                <>
                  <b>Staking:</b> ${Math.round(Number(TVLs.stakingPoolTVL.toSignificant(12)) * 1000000000000)}
                  <br />
                </>
              )}
              {TVLs.totalPitTVL?.greaterThan('0') && (
                <>
                  <b>{pitSettings?.name}:</b> ${Math.round(Number(TVLs.totalPitTVL.toSignificant(12)) * 1000000000000)}
                  <br />
                </>
              )}
              {TVLs.totalCombinedTVL?.greaterThan('0') && (
                <>
                  <b>Total:</b> ${Math.round(Number(TVLs.totalCombinedTVL.toSignificant(12)) * 1000000000000)}
                </>
              )}
            </>
          }
        >
          {TVLs.totalCombinedTVL?.greaterThan('0') && (
            <>TVL: ${Math.round(Number(TVLs.totalCombinedTVL.toSignificant(12)) * 1000000000000)}</>
          )}
        </CustomMouseoverTooltip>
      )}
    </>
